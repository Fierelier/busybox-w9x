#!/bin/sh
# flat sh-only build, no make/kconfig. Assumes `make win9x_defconfig` (or
# another configs/*_defconfig) already ran once. Flags/file list captured
# from a `make -n -B V=1` dry run - regenerate if config changes.
set -e
...
test -f include/autoconf.h || {
	echo "include/autoconf.h missing - run 'make win9x_defconfig' once first." >&2
	exit 1
}
