...
# needed by kconfig's own `source` directive to resolve relative includes
# (e.g. Config.in's "source libbb/Config.in") - must be set before conf runs
srctree="$TOP"
export srctree

# per-directory Config.in/Kbuild aren't checked in (only Config.src/
# Kbuild.src are) - generated by scanning //config:/`//kbuild:` comments
# out of each dir's .c files. Must run before conf, which needs Config.in.
...
test -f include/autoconf.h || {
	# kconfig's conf tool (scripts/kconfig/conf.c + zconf.tab.c, the
	# latter #includes the rest it needs) is plain, buildable host C -
	# no bison/flex/make required. Only the _shipped lexer/parser/hash
	# tables are checked into git though; copy them to the real names
	# kbuild's own generic rule would otherwise produce.
	for f in zconf.tab.c lex.zconf.c zconf.hash.c; do
		test -f "scripts/kconfig/$f" || \
			cp "scripts/kconfig/${f}_shipped" "scripts/kconfig/$f"
	done
	# Run it non-interactively (stdin=/dev/null) to accept every
	# default, same as what `make defconfig` does under the hood.
	DEFCONFIG=${DEFCONFIG:-configs/win9x_defconfig}
	host_build scripts/kconfig/conf -Iscripts/kconfig \
		scripts/kconfig/conf.c scripts/kconfig/zconf.tab.c
	# stdout carries both the ~200-line verbose prompt dump on success
	# AND the tool's own error messages on failure (both go through
	# plain printf, no stderr) - keep it only when something goes wrong.
	if ! conf_out=$(./scripts/kconfig/conf -D "$DEFCONFIG" Config.in < /dev/null 2>&1); then
		echo "$conf_out" >&2
		exit 1
	fi
}
